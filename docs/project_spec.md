...
GitHub Actions 通过独立 `notify.yml` 在北京时间 09:07 调用 `src.notify_email --today`，通过 SMTP 将当天 `output/YYYY-MM-DD.md` 发送给一个或多个收件人。邮件包含纯文本和基础 HTML 两种格式。当天报告缺失时 workflow 失败，避免误发上一日归档。
