...
本项目不是泛 marketing digest，而是面向 quantitative marketing / empirical IO / platform / pricing / AI-enabled marketing research。候选论文在 30 篇截断前必须按 quant relevance 优先保留，避免 consumer psychology / behavioral marketing 论文挤掉核心 quant paper。
...
- 每日最大候选数：30 篇，relevance-first 截断
- 每篇文章请求间隔 ≥ 2 秒（礼貌限速）
...
目的：防止核心 quant paper 被新近但低相关的 behavioral/psych paper 挤出 30 篇上限。
...
[4] 30 篇截断         → 有日期 → quant relevance → 期刊优先级 → 日期↓ → MngSci 营销 tie-breaker → 标题
...
| 异常增量（11~30 篇候选）| < 10 分钟 | 标题 batch；摘要逐篇处理 |
...
| P0 | seen/date 预过滤 + 截断 | ✅ | 日期窗口 + 30 篇 relevance-first 排序 |
