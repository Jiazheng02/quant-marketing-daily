...
标题关键词预判，零 HTTP 调用，仅影响截断排序：

```
MngSci 论文 + 标题含 marketing 关键词 → boost=0（排前）
                                否则 → boost=1（正常排序）
```

目的：防止营销论文在字母序中被非营销论文挤出 30 篇上限。
...
[4] 30 篇截断         → 四级排序：日期↓ → 出版商优先 → MngSci 营销加权 → 标题
...
│   ├── config.py                 # 全局配置
│   ├── dedup.py                  # DOI 去重 · seen 注册表 · 日期窗口 · 截断
...
| `src/dedup.py` | DOI 去重、seen 注册表、日期窗口、截断排序 |
...
| P0 | seen/date 预过滤 + 截断 | ✅ | 日期窗口 + 30 篇四级排序（含 MngSci 营销加权） |
...
| 截断排序 | 三级（日期+优先级+标题） | 四级（+MngSci 营销加权） | 防营销论文字母序挤出 |
