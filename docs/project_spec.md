...
GitHub Actions 在日报生成后调用 `src.notify_email`，通过 SMTP 将最新 `output/YYYY-MM-DD.md` 发送给一个或多个收件人。邮件包含纯文本和基础 HTML 两种格式。HTML 邮件默认使用 `EMAIL_ABSTRACT_MODE=compact`，保留摘要背景框但压缩为预览；`Abstract` 与 `中文翻译` 必须分段显示。邮件客户端不可靠支持交互式 `<details>`，因此不把展开/折叠作为邮件正文的必需能力。
...
| 日期 | 用途 | 来源 |
|---|---|---|
| **`online_date`** | 决定是否进入日报（过滤用） | `prism_coverdate` / Crossref print date / AiA online date |
| **`coverdate`** | 日报中的展示标签 | `prism_coverdate` / Crossref print date |

**提取优先级**：
```
online_date  ←  1. prism_coverdate / Crossref published-print (正式期刊/issue 日期)
                 2. updated_parsed / published-online (Articles in Advance 兜底)
                 3. published_parsed / Crossref published
                 4. "" (不过滤，保守保留)

coverdate    ←  1. prism_coverdate / Crossref published-print (封面/issue 日期)
                 2. updated / published-online (AiA 兜底)
                 3. "" (无日期时不展示标签)
```
...
元数据发现阶段优先使用 RSS；GitHub Actions 上会用 Crossref 补充 Sage/JM/JMR 与 INFORMS/MktSci/MngSci，避免 publisher RSS 被拦截或只返回局部 feed 时漏掉整期文章。Crossref 补充同时查询 print issue date 与 online date，优先用 print issue date 参与日期窗口过滤。
...
```
[1] RSS/Crossref 发现 → papers[]（元数据，无摘要）
...
| `online_date` | str | RSS/Crossref parser | 过滤用日期，优先正式 issue/print 日期 |
| `coverdate` | str | RSS/Crossref parser | 展示用日期，优先正式 issue/print 日期 |
...
├── .github/workflows/daily.yml   # GitHub Actions 凌晨生成日报并发送邮件
├── docs/
...
│   ├── parsers/
│   │   ├── rss_journals.py       # 5 刊 RSS + Crossref fallback 解析
...
| `src/parsers/rss_journals.py` | 5 刊 RSS + Crossref fallback 元数据提取 |
...
| 首跑 / 冷启动 | 推荐先 `--dry-run` | 验证 RSS/Crossref 元数据和截断候选 |
...
- `--dry-run` 模式验证 RSS/Crossref 元数据和 relevance 截断候选
...
| 优先级 | 模块 | 状态 | 说明 |
|--------|------|------|------|
| P0 | RSS/Crossref 解析 + 元数据提取 | ✅ | 5 刊 RSS + Crossref fallback → Paper dict，双日期模型 |
