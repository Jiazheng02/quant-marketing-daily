...
GitHub Actions notify.yml (每日定时 cron: 7 9 * * *, timezone: Asia/Shanghai = 北京时间 09:07)
→ 读取当天 output/YYYY-MM-DD.md
→ SMTP 发送邮件
```
...
GitHub Actions 通过独立 `notify.yml` 在北京时间 09:07 调用 `src.notify_email --today`，通过 SMTP 将当天 `output/YYYY-MM-DD.md` 发送给一个或多个收件人。邮件包含纯文本和基础 HTML 两种格式。当天报告缺失时 workflow 失败，避免误发上一日归档。HTML 邮件默认使用 `EMAIL_ABSTRACT_MODE=compact`，保留摘要背景框但压缩为预览；`Abstract` 与 `中文翻译` 必须分段显示。邮件客户端不可靠支持交互式 `<details>`，因此不把展开/折叠作为邮件正文的必需能力。
...
├── .github/workflows/daily.yml   # GitHub Actions 凌晨生成日报
├── .github/workflows/notify.yml  # GitHub Actions 早上发送邮件
├── docs/
